...
parser.add_argument('--nepoch',default=1)
...
def save_checkpoint(state,args,epoch):
    ...
    file=args.checkpoint_path+'my_checkpoint_time_from_0_%d.pth.tar'%epoch
...
def main(args):
    ...
    if args.load_model:
        load_checkpoint(torch.load(args.checkpoint_path+'my_checkpoint_time_from_0.pth.tar'),netG,netD,optimizer_netG,optimizer_netD)

    criterion = nn.BCELoss().to(device)
    writer=SummaryWriter(f'./runs/out_with_last_frame')
        ...
        for video in range(0,len(video_info)):
            ...
            for i in range(0,frames_num):
                ...
                if i==frames_num-1:#not using the last Key Frame, because the frame num after the last KF is not sure
                    ...
                    # frame_pred,oxes_pred,frame_pred_from_image=netG.frame_generate(img,objs, triples, obj_to_img=None,boxes_gt=boxes,
                    ...
                    if video%5000==0:
                        ...
                        with open(args.generate_frame_path + video_name+'_epoch%d'%epoch+'/triples.txt','w') as f:
                            f.write(str(triples))
                        ...
                        for j, img_pred in enumerate(imgs_pred):
                            ...
                            if j==0:
                                imwrite(args.generate_frame_path+video_name+'_epoch%d'%epoch + '/%d_pred_KF.png' % save_name_pred, img_pred)
                            else:
                                imwrite(args.generate_frame_path + video_name +'_epoch%d'%epoch + '/%d_pred.png' % save_name_pred, img_pred)
                            save_name_pred += 1
                        ...
                        for j, img_real in enumerate(imgs_real):
                            ...
                            imwrite(args.generate_frame_path + video_name +'_epoch%d'%epoch+ '/%d_real.png' % save_name_real, img_real)
                            save_name_real+=1
