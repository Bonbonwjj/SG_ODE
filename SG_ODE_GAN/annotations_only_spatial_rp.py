...
#path set
parser.add_argument('--frames_path', default='./dataset/frames/')
...
def get_info(annotations_path,nonperson_filter=True,vocab=None):
    ...
    kf_info={} #key_frames_info: frames_id:{ bboxes:[np.ndarray] obj_num:[] obj:[] relationship:[]}
    ...
    for id in kf_info:
        ...
        for object in object_bbox_and_relationship[id]:
            if object['visible']==True:
                ...
                # add relationship: each object has a relationship (spatial>contacting) no relationship then set 0 __in_image__
    ...
    for k,v in object_bbox_and_relationship.items():
        if v[0]['metadata']['set']=='train':
            train_list.append(k.split('/')[0])

    train_list=set(train_list)
    print('make train list succesffully')

    # few_frame_train_video_list=[]
    # for t in train_list:
    #     if t in few_frame_video_list:
    #         few_frame_train_video_list.append(t)
    #
    #
    # for v in few_frame_train_video_list:
    #     train_list.discard(v)
    #
    # print('Delete %d frew frame in train list'%len(few_frame_train_video_list))

    return kf_info,video_info,train_list
...
if __name__ == '__main__':
    ...
    # only_contacting=0
    # for v in kf_info.values():
    #     i=0
    #     for n in v['relationship']:
    #         if n<=9:
    #             i+=1
    #             break
    #         else:
