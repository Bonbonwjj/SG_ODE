...
class sgODE_model(nn.Module):
    ...
    def build_netG(self):
        ...
        vae_kwargs={
            'input_channel':128*64*64,
            'h_dim':1024,
            'z_dim':32,
        }
        self.vae_net=VAE(**vae_kwargs).to(self.device)
    ...
    def frame_generate(self, img, objs, triples, obj_to_img=None,
        ...
        input_features=features_combine.view(features_combine.size(0),-1)
        x_reconst, mu, log_var=self.vae_net(input_features)
        features_combine_new=x_reconst.view(-1,128,64,64)

        frame_pred = self.frame_model(features_combine_new)#(num,3,64,64)
        # self.frame_pred=frame_pred

        return frame_pred, boxes_pred,x_reconst,mu,log_var


def compute_losses(args, boxes_pred, boxes_gt,frame_pred,frame_real,mu,log_var):
    ...
    loss_key_img = F.l1_loss(frame_pred[0], frame_real[0])*args.key_img_loss_weight
    loss_img=F.l1_loss(frame_pred[1:],frame_real[1:])*args.img_loss_weight
    # loss_iimg =F.l1_loss(frame_pred_from_image, frame_real)*args.iimg_loss_weight
    total_loss=loss_bbox+loss_key_img+loss_img

    return total_loss


class VAE(nn.Module):
    def __init__(self,input_channel=128*64*64,h_dim=1024,z_dim=32):
        super(VAE,self).__init__()
        self.fc1=nn.Linear(input_channel,h_dim)
        self.fc2=nn.Linear(h_dim,z_dim)
        self.fc3=nn.Linear(h_dim,z_dim)
        self.fc4=nn.Linear(z_dim,h_dim)
        self.fc5=nn.Linear(h_dim,input_channel)
