...
class Discriminator(nn.Module):
    def __init__(self, ndf):
        ...
        self.conv1=nn.Conv2d(ndf*8,ndf*2,3,1,1,bias=False)
        self.conv2=nn.Conv2d(ndf*2,1,4,1,0,bias=False)
        self.LeakyRelu=nn.LeakyReLU(0.2,inplace=True)

    def forward(self, x):

        out = self.conv_img(x)#(batch_size,64,64,64)
        out = self.block0(out)#(batch_size,128,32,32)
        out = self.block1(out)#(batch_size,256,16,16)
        out = self.block2(out)#(batch_size,512,8,8)
        out = self.block3(out)#(batch_size,ndf*8,4,4)
        out=self.conv1(out)
...
class resD(nn.Module):
    ...
    def shortcut(self, x):
        if self.learned_shortcut:
            x = self.conv_s(x)#(batch_size,dim,W,H)→(batch_size,dim,W,H)
        if self.downsample:
            return F.avg_pool2d(x, 2)#W,H→H/2,W/2
        return x
