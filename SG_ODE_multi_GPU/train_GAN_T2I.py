...
from model_T2I import sgODE_model,Discriminator,compute_losses
...
parser.add_argument('--generate_frame_path',default='./generation_SG/')
...
def main(args):
    ...
    writer=SummaryWriter(f'./runs/out_SG')
    step = 0
        ...
        #for video_name,frames_num,key_frame,frame_name,key_frame_inter,obj_list,re_list,bbox_list in train_loader:
            ...
            for i in range(0,frames_num):
                ...
                if i==frames_num-1:#not using the last Key Frame, because the frame num after the last KF is not sure
                    ...
                    output = netD(frame_pred)#(batch_size,1,1,1)
                    loss_netG2=-output.mean()
                    ...
                    disc_real = netD(frame_real)
                    loss_disc_real=torch.nn.ReLU()(1.0-disc_real).mean()

                    disc_fake = netD(frame_pred.detach())
                    loss_disc_fake = torch.nn.ReLU()(1.0+disc_fake).mean()
