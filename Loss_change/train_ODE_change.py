...
parser.add_argument('--generate_frame_path', default='./generation_700_ODE/')
...
# ODE set
parser.add_argument('--tol', type=float, default=1e-2)  # tolerance 1e-3→1e-2
...
def save_checkpoint(state, args, epoch):
    ...
    file = args.checkpoint_path + 'my_checkpoint_700_ODE_%d.pth.tar' % epoch
...
def main(args):
    ...
    writer = SummaryWriter(f'./runs/out_700_ODE')
        ...
        for video in range(0, 700):
            ...
            for i in range(0, frames_num):
                ...
                if i == frames_num - 1:  # not using the last Key Frame, because the frame num after the last KF is not sure
                    ...
                    time_stamps = torch.linspace(start_time * 0.1, end_time * 0.1, steps=steps + 1).float().to(device)
                    #*0.01→0.1
